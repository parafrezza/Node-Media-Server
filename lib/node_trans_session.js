...
class NodeTransSession extends EventEmitter {
  ...
  run() {
    ...
    if (this.conf.hls) {
      ...
      let mapHls = `${this.conf.hlsFlags}${ouPath.split(path.sep).join('/')}/${hlsFileName}|`;
      mapStr += mapHls;
        ...
        if (!err) {
          files.forEach((filename) => 
          {
            if (filename.endsWith('.ts')
              ...
              // console.log('unlnko %s', ouPath + '/' + filename);
              avvueseTePrego.chiudiIlWatch()

            }
          })
        }
        else{console.log('errore chiudendo!')}
      });
    });
  }
